...
export default function ProgressPage({ studentName }) {
  ...
  useEffect(() => {
    if (selectedStudent) loadProgress(selectedStudent);
  }, [selectedStudent]);
  ...
  const handleAddStudent = async () => {
    ...
    try {
      ...
      setMessage('❌ Student may already exist.');
      setTimeout(() => setMessage(''), 3000);
    }
  };
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p>Track quiz scores, chat activity, and learning progress</p>
      </div>
      ...
      <div className="grid-2">
        ...
        <div>
          <div className="card" style={{ marginBottom: 20 }}>
            ...
            {message && (
              <div style={{ background: message.startsWith('✅') ? '#f0fff4' : '#fff5f5', padding: '10px 14px', borderRadius: 8, marginBottom: 12, fontSize: 14, color: message.startsWith('✅') ? '#276749' : '#c53030' }}>
                {message}
              </div>
            )}
            ...
            {showAddForm && (
              <div style={{ background: '#f7fafc', borderRadius: 10, padding: 14, marginBottom: 14 }}>
                ...
                <button className="btn-primary" onClick={handleAddStudent} style={{ padding: '10px 20px', fontSize: 14 }}>
                  Add Student
                </button>
              </div>
            )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <div
                ...
                style={{
                  padding: '12px 14px',
                  background: selectedStudent === studentName ? '#ebf4ff' : '#f7fafc',
                  borderRadius: 10,
                  cursor: 'pointer',
                  border: selectedStudent === studentName ? '2px solid #667eea' : '2px solid transparent',
                  transition: 'all 0.2s'
                }}
              >
              ...
              {students.filter(s => s.name !== studentName).map(s => (
                <div
                  ...
                  style={{
                    padding: '12px 14px',
                    background: selectedStudent === s.name ? '#ebf4ff' : '#f7fafc',
                    borderRadius: 10,
                    cursor: 'pointer',
                    border: selectedStudent === s.name ? '2px solid #667eea' : '2px solid transparent',
                    transition: 'all 0.2s'
                  }}
                >
        ...
        <div>
          {loading && (
            <div className="card" style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <p style={{ color: '#a0aec0' }}>Loading progress...</p>
            </div>
          )}
            ...
            <>
              ...
              <div className="stats-grid" style={{ marginBottom: 20 }}>
                <div className="stat-card">
                  <div className="stat-number">{progress.quiz_stats?.total_quizzes || 0}</div>
                  <div className="stat-label">Quizzes Taken</div>
                </div>
                <div className="stat-card">
                  <div className="stat-number" style={{ color: getScoreColor(progress.quiz_stats?.avg_score || 0) }}>
                    {progress.quiz_stats?.avg_score ? Math.round(progress.quiz_stats.avg_score) : 0}%
                  </div>
                  <div className="stat-label">Average Score</div>
                </div>
                <div className="stat-card">
                  <div className="stat-number" style={{ color: '#48bb78' }}>
                    {progress.quiz_stats?.best_score ? Math.round(progress.quiz_stats.best_score) : 0}%
                  </div>
                  <div className="stat-label">Best Score</div>
                </div>
                <div className="stat-card">
                  <div className="stat-number" style={{ color: '#805ad5' }}>
                    {progress.chat_stats?.total_chats || 0}
                  </div>
              ...
              {progress.quiz_stats?.avg_score > 0 && (
                <div className="card" style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ background: '#e2e8f0', borderRadius: 20, height: 20, overflow: 'hidden' }}>
                    <div style={{
                      height: '100%',
                      width: `${Math.round(progress.quiz_stats.avg_score)}%`,
                      background: `linear-gradient(90deg, ${getScoreColor(progress.quiz_stats.avg_score)}, ${getScoreColor(progress.quiz_stats.avg_score)}aa)`,
                      borderRadius: 20,
                      transition: 'width 0.8s ease',
                      display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: 8
                    }}>
                      <span style={{ color: 'white', fontSize: 12, fontWeight: 700 }}>
                        {getGrade(progress.quiz_stats.avg_score)}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="card">
                ...
                {attempts.length === 0 ? (
                  <p style={{ color: '#a0aec0', textAlign: 'center', padding: '20px 0' }}>No quizzes taken yet</p>
                ) : (
                  attempts.map((a, i) => {
                    ...
                    return (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: i < attempts.length - 1 ? '1px solid #f0f4f8' : 'none' }}>
                        <div>
                          <p style={{ fontWeight: 600, fontSize: 14 }}>{a.topic || 'Quiz'}</p>
                          <p style={{ fontSize: 12, color: '#a0aec0' }}>{new Date(a.created_at).toLocaleDateString()}</p>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <span style={{ background: getScoreColor(pct) + '22', color: getScoreColor(pct), padding: '4px 12px', borderRadius: 20, fontWeight: 700, fontSize: 14 }}>
                            {pct}%
                          </span>
